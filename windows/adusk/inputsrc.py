...
def _sdl_pad_excluded(name):
    """SDL gamepads this source must NOT open:
      * Steam Controller — owned by our custom hidapi driver (opening it in SDL
        too would double-drive it and fight over the HID handle).
      * "Xbox 360 Controller" — our OWN ViGEm virtual pad, created in gamepad
        mode. If we read it back and OR-merge it into the frame we then write to
        it, that's a runaway feedback loop (sticks pin to the extremes, triggers
        and buttons stick, axes ramp — garbage fed to the game). A real Xbox pad
        is already XInput and needs no SDL->ViGEm translation here, so matching
        the name is safe."""
    n = (name or "").lower()
    return "steam" in n or "xbox 360" in n
...
class Sdl3GamepadSource:
    ...
    def __init__(self):
        ...
        # instance_id -> monotonic time that pad last had a button/trigger down.
        # Used to target key-press haptics at the pad actually being used, so a
        # second connected controller doesn't buzz when the first one types.
        self._pad_active_t = {}
        # Desired/applied Home-LED state (None = leave alone). The tray drives
        # this from gamepad mode: lit blue while gamepad mode is active, off
        # otherwise. Applied in poll() on the SDL thread; re-applied to any pad
        # that (re)connects. See set_home_led / _apply_home_led.
        self._home_led_want = None
        self._home_led_applied = None
        # Per-pad: is this pad's LED currently lit for gamepad mode? Used by
        # _update_idle_leds to dim it after _HOME_LED_IDLE_TIMEOUT of no input
        # and re-light it the moment that pad is touched again.
        self._pad_led_lit = {}
    ...
    def _rescan(self):
        try:
            ...
            return
        # Skip the Steam Controller (our hidapi driver owns it) and our own
        # ViGEm virtual Xbox pad (reading it back is a feedback loop). Generic
        # pads — Switch Pro / DualSense / etc. — are kept. See _sdl_pad_excluded.
        names = {jid: (name or "") for jid, name in pads
                 if not _sdl_pad_excluded(name)}
        ids = set(names)
        for jid in list(self._pads):
            if jid not in ids:
                ...
                self._pad_led_lit.pop(jid, None)
        ...
        for jid in ids:
            if jid not in self._pads:
                ...
                if g:
                    ...
                    # A freshly-(re)connected pad starts with its default LED;
                    # force poll() to re-apply our desired Home-LED state to it.
                    self._home_led_applied = None
    ...
    def _pump(self):
        ...
        gamepad state, periodically rescan for (dis)connects, and apply any
        pending Home-LED change. Returns the monotonic 'now', or None when SDL
        is unavailable / the update failed."""
        ...
        if now >= self._next_scan:
            ...
            self._rescan()
        # Apply a pending Home-LED change on the SDL thread (cheap no-op when
        # already applied). Also runs after a (re)connect reset _applied=None.
        if self._home_led_want is not None and self._home_led_want != self._home_led_applied:
            self._apply_home_led()
            if self._home_led_want:
                # Gamepad mode just turned on: count every pad as "just
                # touched" so the LED starts lit and the idle timer below
                # begins counting from now, not from whenever it was last
                # actually used (possibly while gamepad mode was off).
                for jid in self._pads:
                    self._pad_active_t[jid] = now
        if self._home_led_want:
            self._update_idle_leds(now)
        return now
    ...
    def set_home_led(self, on):
        """Request the controller LED lit (blue) or off — tied to gamepad mode.
        Only records intent here; the change is applied on the SDL polling
        thread in poll() (and re-applied to any pad that reconnects)."""
        self._home_led_want = bool(on)

    # Two-pulse intensities for the gamepad-mode toggle confirmation. The light
    # pulse is kept fairly substantial (and a wide gap follows each pulse) so
    # that on the "off" pattern (strong→light) the trailing light pulse isn't
    # masked by the strong pulse's motor spin-down and reads as a distinct 2nd buzz.
    _RUMBLE_LIGHT = (0x4800, 0x4800, 120)   # (low motor, high motor, ms)
    _RUMBLE_STRONG = (0xB000, 0xB000, 160)
    _RUMBLE_GAP = 0.15                       # silence between the two pulses (s)

    def play_mode_rumble(self, on):
        """Buzz a two-pulse confirmation when gamepad mode toggles: light then
        strong on enable, strong then light on disable. Runs on a daemon thread
        so the timed gap between pulses doesn't block the input loop. Gated by
        the SDL pad's (Switch) Vibration toggle."""
        if not state.is_rumble_enabled("sdl"):
            return
        Thread(target=self._mode_rumble_worker, args=(bool(on),), daemon=True).start()

    def _mode_rumble_worker(self, on):
        seq = ((self._RUMBLE_LIGHT, self._RUMBLE_STRONG) if on
               else (self._RUMBLE_STRONG, self._RUMBLE_LIGHT))
        for lo, hi, ms in seq:
            self._rumble(lo, hi, ms)
            time.sleep(ms / 1000.0 + self._RUMBLE_GAP)  # let the pulse finish + a gap

    def _apply_home_led(self):
        # Blue when gamepad mode is on, off otherwise. SDL_SetGamepadLED is a
        # no-op on pads without a settable LED, so this is harmless for others.
        r, g_, b = (0, 0, 255) if self._home_led_want else (0, 0, 0)
        for jid, g in list(self._pads.items()):
            try:
                S.SDL_SetGamepadLED(g, r, g_, b)
            except Exception:
                pass
            self._pad_led_lit[jid] = bool(self._home_led_want)
        self._home_led_applied = self._home_led_want

    # Seconds of no input on a pad before its gamepad-mode LED dims to off.
    # Re-lights instantly the moment that pad sees any button/trigger/stick
    # activity again. Only takes effect while gamepad mode is on overall
    # (_home_led_want); _apply_home_led handles the on/off transition for
    # gamepad mode itself and seeds _pad_active_t so the timer starts fresh.
    _HOME_LED_IDLE_TIMEOUT = 120.0

    def _update_idle_leds(self, now):
        for jid, g in list(self._pads.items()):
            last = self._pad_active_t.get(jid, now)
            want_lit = (now - last) <= self._HOME_LED_IDLE_TIMEOUT
            if self._pad_led_lit.get(jid) != want_lit:
                r, g_, b = (0, 0, 255) if want_lit else (0, 0, 0)
                try:
                    S.SDL_SetGamepadLED(g, r, g_, b)
                except Exception:
                    pass
                self._pad_led_lit[jid] = want_lit
...
class SharedSdlFrameSource:
    ...
    Haptics forward to the tray's live Sdl3GamepadSource (registered in
    state.set_sdl_source). adusk.main polls that same source on its own SDL
    event-pump thread while the OSK is open, so calling SDL_RumbleGamepad from
    here (the key-dispatch path runs on that thread too) hits the active SDL
    thread and the Switch Pro / Xbox / DualSense buzzes on each key press."""
    ...
    def haptic_pad_click(self):
        # No-op for SDL pads. The "strong" pad-click haptic only ever fires for
        # them on the L2/R2 trigger engage (ZL/ZR = Shift/Enter on the Switch
        # Pro) — SDL pads have no trackpad, so the physical-pad-click path that
        # also uses this never applies. The user doesn't want ZL/ZR to buzz, so
        # we simply don't forward it. (Normal key taps still use haptic_click;
        # the Steam Controller's pad-click feedback is unaffected — it goes
        # through SteamHidSource, not this source.)
        return
...
class InputMerger:
    ...
    # controller.py. (The lower _ACTIVITY_STICK is haptics-only, not glyphs.)
