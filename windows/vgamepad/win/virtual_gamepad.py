...
def get_vbus():
    """Return the process-wide ViGEm bus, connecting lazily on first use.

    Upstream vgamepad connects at import time, which makes CI dependency and
    packaging checks fail on machines that do not have the ViGEmBus driver
    installed. SteamlessKeyboard only needs the bus once gamepad mode creates a
    virtual pad, so defer the connection until then.
    """
    global _VBUS
    if _VBUS is None:
        _VBUS = VBus()
