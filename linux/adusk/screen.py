...
# Per-platform font lookup. On Windows we want Segoe UI Semibold + Segoe UI
# Symbol (Steam Big Picture's keyboard font); on Linux we try a few common
# system fonts and fall back to the bundled DejaVu otherwise.
...
class Screen:
    ...
    def __init__(self):
        ...
        font_path = _first_existing(win_candidates + linux_candidates)
        ...
        sym_path = _first_existing(sym_win + sym_linux)
        if sym_path is None:
            sym_path = font_path
    ...
    def set_framerate(self, fps):
        """Change the render-loop frame cap at runtime (used for the adaptive
        active/idle rate in adusk.main)."""
        sdl2.sdlgfx.SDL_setFramerate(ctypes.byref(self.frame_rate_manager), int(fps))
