...
class ControllerManager:
    ...
    def _handle_kbd_stick(self, sc_input, steam_now, now):
        ...
        zone = "NEUTRAL"
        if not steam_now and (abs(x) > self.STICK_DEADZONE
                              or abs(y) > self.STICK_DEADZONE):
            if abs(y) >= abs(x):
                zone = "UP" if y > 0 else "DOWN"
            else:
    ...
    def handle_input(self, sc, sc_input):
        ...
        if steam_now and view_now and not self._start_was_pressed:
            ...
            self._saw_x_during_steam = True
        elif view_now and not self._start_was_pressed:
            # VIEW alone (no Steam) → advance the keyboard through its on-screen
            # position rotation, the same action as the Move key held with Shift.
