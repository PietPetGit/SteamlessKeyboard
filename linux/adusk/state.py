...
# refreshes gamepad state on the event-pump thread, so the tray's
# sdl_gamepad_thread goes blind (reads all-zero / stale frames) once the OSK
# window's event loop is running. The tray cedes while the OSK is open; adusk
# polls here and republishes via set_sdl_frame.
_sdl_source = None
...
# Linux: the Win32 path above is dead (_user32 is None), so read Caps Lock from
# the X server's keyboard modifier mask (LockMask = Caps Lock). XQueryPointer is
# core X11 — no Xkb extension needed — so it works on X11 and XWayland. This is
# what lets the OSK show uppercase labels when L3 toggles Caps Lock (the L3 key
# injects KEY_CAPSLOCK, which toggles the OS lock; we read that lock here). A
# dedicated X connection is opened once and cached; guarded by a lock because
# Xlib connections are not thread-safe.
_LOCK_MASK = 1 << 1            # X11 LockMask = Caps Lock
_x11_caps_state = None         # None=untried, False=unavailable, else (lib, disp, root)
_x11_caps_lock = Lock()


def _x11_caps():
    """(libX11, Display*, root Window) for reading Caps Lock, or None."""
    global _x11_caps_state
    if _x11_caps_state is None:
        try:
            ...
            if not disp:
                _x11_caps_state = False
                return None
            _x11_caps_state = (lib, disp, lib.XDefaultRootWindow(disp))
        except Exception:
            _x11_caps_state = False
    return _x11_caps_state or None
...
def set_sdl_source(src):
    ...
    own SDL event-pump thread while the OSK is open (see _sdl_source)."""
    global _sdl_source
    _sdl_source = src
...
def set_active_controller_persist(fn):
    ...
    active controller changes, so the tray can save it to its settings file."""
...
def is_caps_on():
    ...
    if _user32 is not None:  # Windows
        return bool(_user32.GetKeyState(_VK_CAPITAL) & 0x0001)
    # Linux/X11: read the keyboard modifier mask (LockMask = Caps Lock).
    with _x11_caps_lock:
        st = _x11_caps()
        if st is None:
            return False
        ...
        try:
            ...
            return bool(mask.value & _LOCK_MASK)
        except Exception:
            return False
