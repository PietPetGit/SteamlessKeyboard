...
# OS-level Caps Lock state. The OSK reads this every render so the displayed
# glyphs match whatever the OS thinks. L3 just sends KEY_CAPSLOCK to the OS
# rather than tracking caps locally — the OS is the source of truth.
#
# Windows: user32.GetKeyState(VK_CAPITAL) — reads the actual latched state.
# Linux: track manually. KWin under a Wayland session doesn't propagate
#   keyboard-lock state to XWayland's XKB, and the sysfs LED nodes only
#   update for devices that declared LED capability (our uinput virtual
#   keyboard doesn't). So instead, the uinput backend in
#   steamcontroller/uinput.py calls notify_caps_key_sent() each time it
#   injects KEY_CAPSLOCK; we flip _caps_tracked on each call. Drifts if
#   the user toggles caps from a real keyboard while the OSK is open —
#   acceptable, the OSK use case is controller-driven.
_VK_CAPITAL = 0x14
try:
...
def notify_caps_key_sent():
    """Called by the uinput backend on every KEY_CAPSLOCK press it sends.
    Toggles the tracked Linux caps state."""
    global _caps_tracked
    with _caps_lock:
        _caps_tracked = not _caps_tracked
