...
# SDL_SetWindowPosition (and Show/AlwaysOnTop) can be asynchronous on X11/
# XWayland: the request is queued and applied by the WM on its own schedule.
# SDL_SyncWindow blocks (briefly) until pending window-manager changes have
# actually taken effect -- used by the open animation, which calls
# SetWindowPosition every frame and would otherwise present a frame whose
# content assumes a position the WM hasn't applied yet (visible as flicker).
SDL_SyncWindow = _bind(SDL, "SDL_SyncWindow", ctypes.c_bool, [ctypes.c_void_p])
...
def get_win32_hwnd(window):
    ...
    (Always None on Linux — the win32 property is absent; kept for parity.)"""
...
# X11 window properties (Linux/XWayland). The display is a pointer; the window
# is the X11 Window XID as a number. Used by adusk.py to set the OSK's
# no-keyboard-focus WM hints via libX11 directly.
SDL_PROP_WINDOW_X11_DISPLAY_POINTER = b"SDL.window.x11.display"
SDL_PROP_WINDOW_X11_WINDOW_NUMBER = b"SDL.window.x11.window"
SDL_GetNumberProperty = _bind(SDL, "SDL_GetNumberProperty", ctypes.c_int64,
                              [SDL_PropertiesID, ctypes.c_char_p, ctypes.c_int64])


def get_x11_display(window):
    """Return the X11 Display* (as an int) backing an SDL window, or None."""
    props = SDL_GetWindowProperties(window)
    if not props:
        return None
    disp = SDL_GetPointerProperty(props, SDL_PROP_WINDOW_X11_DISPLAY_POINTER, None)
    return int(disp) if disp else None


def get_x11_window(window):
    """Return the X11 Window XID (as an int) backing an SDL window, or None."""
    props = SDL_GetWindowProperties(window)
    if not props:
        return None
    xid = SDL_GetNumberProperty(props, SDL_PROP_WINDOW_X11_WINDOW_NUMBER, 0)
    return int(xid) if xid else None
...
# SDL docs mark this "main thread only" for the video/Cocoa event pump, but on
# Linux with no SDL_INIT_VIDEO window the joystick/gamepad subsystems have no
# such restriction — Sdl3GamepadSource calls this from the tray's gamepad
# thread purely to drive udev hotplug detection (see _pump in inputsrc.py).
SDL_PumpEvents = _bind(SDL, "SDL_PumpEvents", None, [])
